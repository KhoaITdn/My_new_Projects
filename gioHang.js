...
function addProductToCart(user) {
    ...
    for (var i = 0; i < user.products.length; i++) {
        ...
        <div class="cart__product">
                <a target="_self" href="chiTietSanPham.html?` + p.name.split(' ').join('-') + `" class="cart-product__link">
                    <img src="` + p.img + `" alt="Ảnh cỏ may mắn"
                        class="cart-product__link-img">
                </a>
                <a target="_self" href="chiTietSanPham.html?` + p.name.split(' ').join('-') + `" class="cart-product__name-link">` + p.name + `</a>
