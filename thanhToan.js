...
function addProductToPayment(user) {
    ...
    for (var i = 0; i < user.products.length; i++) {
        ...
        <div class="transaction__product">
            <a target="_self" href="chiTietSanPham.html?` + p.name.split(' ').join('-') + `" class="transaction-product__link">
                <img src="` + p.img + `" alt="Ảnh cỏ may mắn"
                    class="transaction-product__link-img">
            </a>
            <a target="_self" href="chiTietSanPham.html?` + p.name.split(' ').join('-') + `" class="transaction-product__name-link">` + p.name + `</a>
